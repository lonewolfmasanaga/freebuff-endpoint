...
await t('concurrent acquires serialize rotation (one startRun)', async () => {
  ...
  rm.pools = [{ token: 'TOKEN-AAAAAA', runs: new Map(), cooldownUntil: 0, lastError: null }];
  const [l1, l2] = await Promise.all([
    rm.acquirePool(rm.pools[0], 'agent-x', null),
    rm.acquirePool(rm.pools[0], 'agent-x', null),
  ]);
...
await t('rotation finishes old run once and starts new', async () => {
  ...
  rm.pools = [
    {
      token: 'TOKEN-BBBBBB',
      runs: new Map([['ag', { id: 'old', startedAt: Date.now() - 999 * 60000, inflight: 0, requests: 2 }]]),
      cooldownUntil: 0,
      lastError: null,
    },
  ];
  const lease = await rm.acquirePool(rm.pools[0], 'ag', null);
  assert.equal(lease.run.id, 'r1');
  assert.deepEqual(finished, ['old'], 'old run FINISHed exactly once');
  assert.equal(rm.pools[0].runs.get('ag').id, 'r1');
});
...
await t('shutdown drains in-flight before FINISH, rejects new acquires', async () => {
  ...
  rm.pools = [
    {
      token: 'TOKEN-CCCCCC',
      runs: new Map([['ag', { id: 'live', startedAt: Date.now(), inflight: 1, requests: 1 }]]),
      cooldownUntil: 0,
      lastError: null,
    },
  ];
  ...
  rm.release({ run: rm.pools[0].runs.get('ag') }); // simulate request completing
  await shut;
  assert.deepEqual(fin, ['live'], 'FINISH only after in-flight drained');
